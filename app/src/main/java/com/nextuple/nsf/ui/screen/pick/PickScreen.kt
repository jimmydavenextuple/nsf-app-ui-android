...
// 				).show()
			...
			pickVM.pickTask != null || pickVM.sfsPickTask != null -> {
				val currentTask = pickVM.pickTask ?: pickVM.sfsPickTask
				currentTask?.let { task ->
					...
					PickDetailsScreen(
						...
						subFulfillmentType = task.subFulfillmentType,
						currentPickTaskItem = pickVM.currentPickItem,
						unitsWorked = task.totalWorkedQty ?: 0,
						totalUnits = task.totalQty ?: 0,
		...
		if (
			...
			Column(
				...
				CircularProgressBar(
					...
					totalUnits = totalUnits,
					inProgressUnits = inProgressUnits ?: 0,
					centerText = "TOTAL SFS UNITS WORKED",
					centerIconModifier = Modifier.clickable { dialogVisibility(true) }
				)
