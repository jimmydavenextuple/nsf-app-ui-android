...
class InfoServiceTest {
	...
	fun `getStoreOverview should call api with the expected values`() = runTest {
		...
		verify {
			runBlocking { infoApi.getStoreOverview(store.id, userId) }
		}
	}
	...
	@Test
	fun `getStoreOverview should return general error if getUserId is null`() = runTest {
		every { runBlocking { userRepository.getStore() } } returns Store(id = "456", brand = Brand.NT_BRAND_A)
	...
	@Test
	fun `getDeclineCodes should return general error if getUserId is null`() = runTest {
		every { runBlocking { userRepository.getStore() } } returns Store(id = "456", brand = Brand.NT_BRAND_A)
		every { runBlocking { userRepository.getUserId() } } returns null
